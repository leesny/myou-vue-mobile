...
const STORE_PREFIX = '_SW_' + Constant.env.NODE_ENV.toUpperCase() + "_"
let storage = {
  ...
  session: {
    set(key, value) {
      ...
      try {
        ...
        if (console) console.warn("sessionStorage didn't successfully save the '{" + key + ": " + value + "}' pair, because the sessionStorage is full.");
      }
    },
    get(key) {
      if (!key) return null
