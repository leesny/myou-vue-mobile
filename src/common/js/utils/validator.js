...
var _testHook = {
  ...
  // 验证身份证号码
  is_idCard: function(field) { return regexs.idCard.test(backVal(field)); },
  // 验证URL
  ...
  // 验证  大于等于0
  is_positive: function(field) {
    return regexs.positive.test(backVal(field)) || backVal(field) == 0;
  },
  // 是否为必填
  ...
  // textarea 最大长度   默认256
  textarea_max_length: function(field) {
    return (backVal(field).length <= parseInt(256, 10));
  },

  // input  最大长度   默认 30
  input_max_length: function(field) {
    return (backVal(field).length <= parseInt(30, 10));
  },
  ...
  // 用于服务条款,是否时相当有用,不限制checkbox与radio,有可能submit button直接附带value情况
...
//表单字段验证-基于表单字段元素
export const validateField = function(field) {
  if (!field || !field.rules) {
    return [];
  }

  var rules = field.rules.split('|'),
    isEmpty = (field.value !== 0 && (!field.value || field.value === '' || field.value === undefined));
  ...
  for (var i = 0, ruleLength = rules.length; i < ruleLength; i++) {
    ...
    if (failed) {
      var message = (function() {
        return field.ruleTexts.split('|')[i] && field.ruleTexts.split('|')[i].replace('{{' + field.name + '}}', field.value)
      })()

      var existingError, j;
      for (j = 0; j < errors.length; j += 1) {
        if (field.element === errors[j].element) {
          existingError = errors[j];
        }
      }

      var errorObject = existingError || {
        ruleTexts: field.ruleTexts,
        name: field.name,
        message: message,
        messages: [],
        state: false,
...
//表单字段验证- 基于表单字段值
export const validateFieldVal = function(fieldVal) {
  if (!fieldVal || !fieldVal.rules) {
    return [];
  }

  var rules = fieldVal.rules.split('|'),
    isEmpty = (fieldVal.value !== 0 && (!fieldVal.value || fieldVal.value === '' || fieldVal.value === undefined));
  ...
  for (var i = 0, ruleLength = rules.length; i < ruleLength; i++) {
    ...
    if (typeof _testHook[method] === 'function') {
      if (!_testHook[method].apply(this, [fieldVal, param])) {
        failed = true;
      }
    }
    if (regexs[method] && /^regexp\_/.test(method)) {
      if (!regexs[method].test(fieldVal.value)) {
        failed = true;
      }
    }
    if (failed) {
      var message = (function() {
        return fieldVal.ruleTexts&&fieldVal.ruleTexts.split('|')[i] && fieldVal.ruleTexts.split('|')[i].replace('{{' + fieldVal.name + '}}', fieldVal.value)
      })()

      var existingError, j;
      for (j = 0; j < errors.length; j += 1) {
        if (fieldVal.element === errors[j].element) {
          existingError = errors[j];
        }
      }

      var errorObject = existingError || {
        ruleTexts: fieldVal.ruleTexts,
        name: fieldVal.name,
        message: message,
        messages: [],
        state: false,
...
export const validateForm = function(data) {
  if (data == null || typeof data !== 'object') {
    console.error('表单数据必须是Object！请检查。');
    return false;
  }

  let failedField = Object.values(data).find(item => {
    return item.rules && item.rules.length && (!item.state || (item.rules.includes('required') && isEmpty(item.value)));
  });

  failedField && console.log('表单验证未通过： >>>', failedField);
  if (failedField) {
    //feedback 反馈信息
    if (!failedField.feedback) {
      let currError = '信息有误，请检查！';
      let errs = validateFieldVal(failedField);
      if (errs && errs.length > 0) {
        currError = errs[0].message;
      }

      failedField.feedback = currError;
    }
